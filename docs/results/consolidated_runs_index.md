...
見 [inference_families.md](inference_families.md)。四個家族加一層探索性，依「在看到
任何結果之前它被寫在哪份文件裡」歸類，不依效果大小。三項對 C 直接有影響：

- **Table 3 改報七個方法**，不報目前的三列 —— 「最佳投影／最佳解碼」是看過分數才選的。
- **C-wF1 與 hF 降為敘述**，不再各自成 Holm 家族（移除 10 個檢定，論證強度不變）。
- **Tuple exact-match 獨立成家族**，與官方指標分開套 Holm。
