...
**最後更新 2026-08-25（C）** · 對應 commit 見 `tables/manifest.json` 的 `git_sha`
...
| 2. 方法增益小，但評估目標落差大且穩定 | ✅ **成立** | 三個區間全部排除 0；落差 0.012–0.015 > 方法間全距 0.0082 |
...
⚠️ **這裡有一段必須寫進 Methods 的修正。** 早期草稿還把 `M6-M5` 在官方指標上的 `-0.008 [-0.016, -0.001]` 當成「顯著較差」。**那是錯的**——它的 `p_Holm = 0.171`，過不了校正。錯誤來源是我們**用未校正的百分位區間判定顯著性，卻在同一句話裡說「Holm 校正過」**。已於 8/22 修正，詳見 Part II §7.5。
...
**三個區間全部排除 0**，而七個方法在 weighted F1 上的全距只有 **0.0082**。
...
| M3-M2 | conditional vs global（projection 下） | +0.001 [-0.004, 0.006] | 1.000 |
| M4-M1 | 17-state vs projection | +0.000 [-0.005, 0.005] | 1.000 |
| M6-M3 | 在 conditional 下加 decoder | -0.008 [-0.018, 0.002] | 0.416 |
| M6-M5 | conditional vs global（decoding 下） | -0.008 [-0.016, -0.001] | **0.171** |

> **五組全部無法解析。** 注意最後一列：區間排除 0，但 `p_Holm = 0.171`。**不可寫成顯著。**
...
⚠️ **但這三筆不是三份獨立的證據，論文不可以那樣寫。** 它們共享同一批預測、同一批重抽，而且在數學上相關。C-wF1 上的那一筆更可以**逐項還原**成官方指標的結果：

> `C-wF1(M1−M0)  ≡  官方(M1−M0)  +  M0 被取消的部分分數`
> `+0.0039  =  −0.0011  +  0.0050`（document-disjoint）

**也就是說，C-wF1 上的顯著不是一次新的量測，而是一次重新記帳。** 把 M0 靠「祖先不成立的欄位」收下的 0.0050 拿掉之後，原本 −0.0011 的落後就翻成 +0.0039 的領先。分子分母都沒變，變的是哪些欄位有資格計分。

⚠️ **能歸因的只有 C-wF1。** 另外兩個各自多改了一件事（見 §8），只能當佐證。

**論文可以主張的最強版本**：一個既有的計分原則套用在官方指標上，會**翻轉**同一批預測的結論；而翻轉的幅度恰好等於無約束基線靠違反階層的欄位收下的分數。**不要寫成「三把尺都同意」——那是 overclaim，而且審稿人會拿恆等式打回來。**
...
|---|---|---|---|---|
| M6-M5 | 官方 wF1 | 顯著較差 | 0.171 | 無法解析 |
| M6-M5 | C-wF1 | 顯著較差 | 0.137 | 無法解析 |
| M4-M1 | hF | 顯著較好 | 0.135 | 無法解析 |
| M6-M3 | hF | 顯著較好 | 0.224 | 無法解析 |
...
| M6-M3 | hF | +0.006 | 0.002 ✅ | 主 protocol 沒過（0.224） |
...
**為什麼？已經量測過了，而且直覺的解釋是錯的。**

早期草稿猜的是：same-document 的列來自模型已經部分看過的報告，違反階層的預測本來就比較少。**量測結果相反**——same-document 的 M0 非法率是 **12.90%**，比 document-disjoint 的 **12.55%** 還**高**。

真正的分解（因為合法方法的 C-wF1 等於官方分數，所以這是恆等式，不是近似）：

| Protocol | M0 非法率 | 官方 (M1−M0) | M0 被取消的部分分數 | C-wF1 (M1−M0) |
|---|---|---|---|---|
| document-disjoint（主） | 12.55% | −0.0011 | +0.0050 | **+0.0039** |
| same-document | 12.90% | −0.0038 | +0.0056 | **+0.0018** |

**被取消的部分分數在兩個 protocol 上是穩定的**（0.0050 vs 0.0056）；**變的是投影本身在官方指標上的代價**（−0.0011 vs −0.0038）。

換句話說：不是 same-document 的階層違規比較少，而是**在那裡修復它們要付出更多代價**。這一段由 `analysis/findings.py::_decomposition` 生成，數字見 `tables/findings.md` §4。
...
| M3 | 0.796 | 0.501 / 0.475 | 0.651 / 0.669 | 0.416 / 0.373 |
| M4 | 0.799 | 0.468 / 0.431 | 0.648 / 0.665 | 0.420 / 0.380 |
| M5 | 0.796 | 0.493 / 0.468 | 0.649 / 0.670 | 0.423 / 0.387 |
| M6 | 0.784 | 0.494 / 0.479 | 0.634 / **0.664** | 0.418 / 0.397 |
...
**敏感度**：移除那兩列後，七個方法的官方分數**一致上升約 +0.049**（M0 0.5723 → 0.6220，M6 0.5674 → 0.6163，全距僅 0.0015）。
...
`[-0.016, -0.001]` 的意思是：**在只做這一次比較的前提下**，真實差距有 95% 的機會落在這個範圍。
...
> Δ = −0.008（95% CI [−0.016, −0.001]，uncorrected；p_Holm = 0.171，**not significant** after Holm correction across the five pre-specified contrasts）
...
| 區間排除 0 就是顯著 | 區間是**未校正**的；判定看 `p_Holm`。`M6-M5` 官方指標 -0.008 [-0.016, -0.001] 的 `p_Holm` 是 0.171 |
...
## 18.5 ⚠️ 這篇論文最可能被怎麼攻擊，以及我們的回答

**這一節比上面的 15 題重要。** 上面那些是可預期的技術問題；下面這個是**針對研究設計本身**的攻擊，而且它站得住一部分。**論文必須自己先講，不能等審稿人講。**

### 攻擊

> 你們設計了方法、訓練完、分析完，發現在官方指標上沒有提升。於是你們換了一個評估指標——包含對你們方法有利的整列全對率——然後宣稱有顯著提升。這不就是繞一個彎、把方法跟評估都換掉，來在一個資料集上製造顯著嗎？

### 我們必須承認的三件事（全部寫進論文，不迴避）

1. **指標是事後採用的。** C-wF1、hF 都不在預先指定的分析計畫裡，是看過主要結果之後才加的。**必須在出現這些數字的每一個地方載明。**
2. **三個結構感知指標不是三份獨立證據。** 同一批預測、同一批重抽，數學上相關。
3. **最關鍵**：`C-wF1(M1−M0) ≡ 官方(M1−M0) + M0 被取消的部分分數`。這是**重新記帳，不是新的量測**（見 §7.3）。

### 我們守得住的四件事

1. **結果不是套套邏輯。** 如果「換成階層感知指標」必然製造顯著，那它在 same-document protocol 上也該顯著——但 **p_Holm = 0.739，沒有重現**（§9）。指標本身不保證結論。
2. **指標沒有一面倒地對我們有利。** `M4-M1` 在 tuple accuracy 上**顯著更差**（p_Holm=0.028），而我們留著它。計畫事前指定了這個家族，**看到方向不利就撤掉，不論方向都是選擇性報告**。
3. **抵銷是我們自己報出來的發現**（§3⑧、§5.2 的逐類別帳），不是被人抓出來的漏洞。一份想製造顯著的論文不會主動把「我們的收益全落在最便宜的類別」寫進 Discussion。
4. **真正的爭點在任務，不在指標。** 「`promise_status=No` 卻填了 `verification_timeline=already`」這種答案，在官方計分下拿到 **42.9%** 的部分分數。**這樣的答案值不值 42.9%？** 這是一個關於任務語意的問題。我們的貢獻是把這個問題變成**可量化的**，不是宣稱自己的答案更對。

### 這條界線在哪裡

**metric shopping** = 換指標換到顯著，然後只報那一個。
**metric study** = 四個家族**全部完整報告**、包含對自己不利的、並標明哪些是事後採用。

我們在後者，**但這是靠「全部報告」守住的，不是靠運氣**。任何一次「這個家族結果不好，拿掉吧」都會讓這篇論文掉到前者。

### 寫作指示

⚠️ **Abstract 第一句就要承認官方指標上的 null**，不要藏到 Results 才講。
審稿人最反感的是「讀到第 4 頁才發現主張是空的」；反過來，一開始就誠實會讓後面的分析更可信。

⚠️ **不要試圖把它救成正面結果。** 現在的證據支撐不了，硬寫會在審稿時整片倒下。

**我們的位置其實比純負面結果好，有三樣東西是純 null 沒有的：**

| | 為什麼重要 |
|---|---|
| 一個**不需要統計**的保證：12.72% → 0 | 構造性質，不會被「49 個 cluster 檢定力不足」打掉 |
| 一個**機制**：抵銷的逐類別帳 | 「試了沒用」不是論文；「試了沒用，**因為修復落在最便宜的類別、損害落在最貴的類別，在等權重平均下抵銷**」是論文 |
| 一個**獨立於方法**的發現：評估切分落差（0.012–0.015）> 七個方法全距（0.0082） | 它不依賴我們的方法成立與否 |
...
### 24.4 可重現性：`-dirty` 標記看不見產生表格的程式

`paper/provenance.py::CODE_PATHSPEC` 決定 `git_sha` 的 `-dirty` 後綴要對哪些路徑生效。它列的是 `paper`、`contracts`、`environment.yml`、`pyproject.toml`——**漏了 `analysis`**。

但 `analysis/` 正是產生 `tables/audit.json`、`case_analysis.json`、`findings.md`、`manifest.json` 這四個帶 provenance 戳記檔案的程式。所以在 8/22 之前，**只要 `analysis/` 有未提交的修改，產生出來的交付物都會帶著一個「乾淨」的 `git_sha`，而那個 commit 並不是真正產生它的程式**。

⚠️ **更值得記的是為什麼沒被抓到。** `tests/test_provenance.py` 裡有一個測試就叫 `test_the_pathspec_covers_every_directory_holding_study_code`，docstring 寫著「新的頂層程式目錄會靜默地不再被監看」——但它的斷言是 `{"paper", "contracts"} <= included`，**寫死清單而不是從檔案樹推導**。名字承諾的性質，斷言沒有檢查。

已修正：`CODE_PATHSPEC` 加入 `analysis`，並把該測試改成**從 repo 的目錄結構推導**出所有含 `.py` 的頂層目錄（`tests` 除外，它不可能改變交付物）。
