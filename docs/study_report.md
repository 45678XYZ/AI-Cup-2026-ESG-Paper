...
**七個區間全部排除 0**，Δ 的範圍是 0.010–0.020，而七個方法在 weighted F1 上的全距只有 **0.0088**。

換評估設定造成的差異，**大於換方法造成的差異** —— 而且**最小的那個 Δ（0.010）都比整個方法全距大**。七個方法方向一致，證明落差是 benchmark 切分的性質，不是某個方法的特性。

⚠️ **這張表原本只報三列**（M0 加上「最佳投影」與「最佳解碼器」）。那兩個是**在同一張表裡依分數挑出來的**，屬於表內事後選擇。改成七法全報之後結論不變，反而更強 —— 依 `docs/inference_families.md`，這是一個**七項的預先指定家族**。
...
| 一個**獨立於方法**的發現：評估切分落差（0.012–0.015）> 七個方法全距（0.0088） | 它不依賴我們的方法成立與否 |
